// fortune_docker/frontend/src/components/buttons/useDyslexiaTheme.js
...
const useDyslexiaTheme = () => {
  ...
  if (!context) {
    throw new Error('useDyslexiaTheme must be used within ThemeDyslexiaProvider');
  }
  return context;
};
...
const ThemeDyslexiaProvider = ({ children }) => {
  ...
  const toggleDyslexiaTheme = () => {
    ...
    isDyslexia ? bodyEl.classList.add('dyslexia') : bodyEl.classList.remove('dyslexia');
